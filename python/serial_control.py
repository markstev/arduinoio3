...
def MessageToBytes(address, command, timeout=0, send_checksums=True):
  #command += "\0"
  len_addr = 1
  ...
  for c in command:
    as_bytes.append(c)
  if send_checksums:
      first_checksum, second_checksum = Fletcher16Checksum(as_bytes)
...
class Message:
  ...
  def AddByte(self, byte):
    if not self.has_address_length:
      ...
      self.byte_index += 1
      logging.info('At byte %d of %d', self.byte_index, self.address_length + self.command_length)
  ...
  def Write(self, address, command, timeout=0):
    with self.write_mutex:
      ...
      for retry in range(1000):
        ...
        for byte in MessageToBytes(address, command, timeout):
          if time.time() - t > 1:
